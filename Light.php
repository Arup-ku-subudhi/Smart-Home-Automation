...
<h1 class="justify-content-left text-primary" style="font-weight:300;"><b><strong>My Smart Home</strong></b></h1>
  <ul class="navbar-nav" >
    ...
    <li class="nav-item active">
      <a class="nav-link" href="#"><h4>Light</h4></a>
    </li>
    <li class="nav-item">
...
<form method="POST" action="Light.php" align="center"><br><br>
<div>
	
	<button type="submit" name="Slight" class="btn btn-dark" style="padding:5px 20px;"><h6>Light</h6></button>
	<?php 
		$st=1;
		...
		<?php
			...
			if(isset($_POST['sdate'])){
				if($_POST['date'] == null){
					...
					$d = $_POST['date'];
					$user_check_query = "SELECT * FROM light_tb WHERE Date = '$d'";
					$results  = mysqli_query($db,$user_check_query);
					$num_rows = mysqli_num_rows($results);
					$user = mysqli_fetch_assoc($results);
					
					//echo $user['Id'],date("H",strtotime($user['OTime']));
					...
					{	
						...
						{	
							$user_check_query = "SELECT * FROM light_tb WHERE Date = '$d' AND Id = '$i'";
							$results  = mysqli_query($db,$user_check_query);
							$user = mysqli_fetch_assoc($results);
							?>
							<div class="container">
